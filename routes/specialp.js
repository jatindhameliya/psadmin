...
routes.get('/',(req, res) => {
    if(req.session.userid != "" && req.session.userid != undefined){
        ...
        MongoUtility.GetWithSort('special_products', sortObj, (err, product_result) => {
            if(err){
                console.log(err);
            }else{
                if(product_result){
                    ...
                    async.forEachSeries(product_result, function (product, next_product) {
                        ...
                        res.render('pages/specialproduct_list',{'products' : allproduct, 'ociurl' : process.env.S3_IMAGE_URL});
                    });
                }else{
                    res.render('pages/specialproduct_list',{'products' : [], 'ociurl' : process.env.S3_IMAGE_URL});
                }  
            }
        });
    }else{
            ...
            if(err){
                ...
                if(category_result){
                    ...
                    async.forEachSeries(category_result, function (category, next_category) {
                        ...
                        res.render('pages/specialproduct_create',{'s3_image_url' : process.env.S3_IMAGE_URL, 'categorys' : allcategory, 'ociurl' : process.env.S3_IMAGE_URL});
                    });
                }else{
                    res.render('pages/specialproduct_create',{'s3_image_url' : process.env.S3_IMAGE_URL, 'categorys' : [], 'ociurl' : process.env.S3_IMAGE_URL});
                }  
            }
        });
    }else{
                            ...
                            if(err){
                                ...
                                if(category_result){
                                    ...
                                    async.forEachSeries(category_result, function (category, next_category) {
                                        ...
                                        res.render('pages/specialproduct_edit',{'data' : arr, 's3_image_url' : process.env.S3_IMAGE_URL, 'categorys' : allcategory, 'ociurl' : process.env.S3_IMAGE_URL});
                                    });
                                }else{
                                    res.render('pages/specialproduct_edit',{'data' : arr, 's3_image_url' : process.env.S3_IMAGE_URL, 'categorys' : [], 'ociurl' : process.env.S3_IMAGE_URL});
                                }  
                            }
                        });
                    } else {
...
routes.post('/update',(req, res) => {
    if(req.session.userid != "" && req.session.userid != undefined){
        ...
        MongoUtility.Update('special_products', updateObj, arr, (err, productresult) => {
            if (err) {
...
routes.post('/addNew',(req, res) => {
    if(req.session.userid != "" && req.session.userid != undefined){
        ...
        MongoUtility.SelectOne('special_products', queryObj_product_name, (err, product_result) => {
            if (err) {
                ...
                if (product_result) {
                    ...
                    MongoUtility.Insert('special_products', arr, (err, productresult) => {
                        if (err) {
                            req.flash("error_messages", "Something went wrong, Product can not be added please try again.");
                            req.flash("success_messages", "");
                            var goto = appURI + '/specialp';
                            ...
                            MongoUtility.SelectOne('specialproductgraph', totalpgraphObj, (err, allproductgraphresult) => {
                                if(allproductgraphresult){
                                    if(allproductgraphresult.hasOwnProperty(time)){
                                        ...
                                        MongoUtility.Update('specialproductgraph', totalpgraphObj, arr, (err, updateresult) => {
                                            if (err) {
                                                console.log(err);
                                            }else{
                                                req.flash("error_messages", "");
                                    ...
                                    MongoUtility.Insert('specialproductgraph', arr, (err, updateresult) => {
                                        if (err) {
                                            console.log(err2);
                                        } else {
                                            req.flash("error_messages", "");
...
routes.get('/remove', (req, res) => {
    if(req.session.userid != "" && req.session.userid != undefined){
        if(req.query.id != "" && req.query.id != undefined){
            var queryObj = { '_id': new ObjectId(req.query.id)};
            MongoUtility.Delete('special_products', queryObj, (err, product) => {
                if (err) {
