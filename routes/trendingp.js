...
routes.get('/',(req, res) => {
    if(req.session.userid != "" && req.session.userid != undefined){
        ...
        MongoUtility.GetWithSort('trending_products', sortObj, (err, product_result) => {
            if(err){
                console.log(err);
            }else{
                if(product_result){
                    ...
                    async.forEachSeries(product_result, function (product, next_product) {
                        ...
                        res.render('pages/trendingproduct_list',{'products' : allproduct, 'ociurl' : process.env.S3_IMAGE_URL});
                    });
                }else{
                    res.render('pages/trendingproduct_list',{'products' : [], 'ociurl' : process.env.S3_IMAGE_URL});
                }  
            }
        });
    }else{
            ...
            if(err){
                ...
                if(category_result){
                    ...
                    async.forEachSeries(category_result, function (category, next_category) {
                        ...
                        res.render('pages/trendingproduct_create',{'s3_image_url' : process.env.S3_IMAGE_URL, 'categorys' : allcategory, 'ociurl' : process.env.S3_IMAGE_URL});
                    });
                }else{
                    res.render('pages/trendingproduct_create',{'s3_image_url' : process.env.S3_IMAGE_URL, 'categorys' : [], 'ociurl' : process.env.S3_IMAGE_URL});
                }  
            }
        });
    }else{
                            ...
                            if(err){
                                ...
                                if(category_result){
                                    ...
                                    async.forEachSeries(category_result, function (category, next_category) {
                                        ...
                                        res.render('pages/trendingproduct_edit',{'data' : arr, 's3_image_url' : process.env.S3_IMAGE_URL, 'categorys' : allcategory, 'ociurl' : process.env.S3_IMAGE_URL});
                                    });
                                }else{
                                    res.render('pages/trendingproduct_edit',{'data' : arr, 's3_image_url' : process.env.S3_IMAGE_URL, 'categorys' : [], 'ociurl' : process.env.S3_IMAGE_URL});
                                }  
                            }
                        });
                    } else {
...
routes.post('/update',(req, res) => {
    if(req.session.userid != "" && req.session.userid != undefined){
        ...
        MongoUtility.Update('trending_products', updateObj, arr, (err, productresult) => {
            if (err) {
...
routes.post('/addNew',(req, res) => {
    if(req.session.userid != "" && req.session.userid != undefined){
        ...
        MongoUtility.SelectOne('trending_products', queryObj_product_name, (err, product_result) => {
            if (err) {
                ...
                if (product_result) {
                    ...
                    MongoUtility.Insert('trending_products', arr, (err, productresult) => {
                        if (err) {
                            req.flash("error_messages", "Something went wrong, Product can not be added please try again.");
                            req.flash("success_messages", "");
                            var goto = appURI + '/trendingp';
                            ...
                            MongoUtility.SelectOne('trendingproductgraph', totalpgraphObj, (err, allproductgraphresult) => {
                                if(allproductgraphresult){
                                    if(allproductgraphresult.hasOwnProperty(time)){
                                        ...
                                        MongoUtility.Update('trendingproductgraph', totalpgraphObj, arr, (err, updateresult) => {
                                            if (err) {
                                                console.log(err);
                                            }else{
                                                req.flash("error_messages", "");
                                    ...
                                    MongoUtility.Insert('trendingproductgraph', arr, (err, updateresult) => {
                                        if (err) {
                                            console.log(err2);
                                        } else {
                                            req.flash("error_messages", "");
...
routes.get('/remove', (req, res) => {
    if(req.session.userid != "" && req.session.userid != undefined){
        if(req.query.id != "" && req.query.id != undefined){
            var queryObj = { '_id': new ObjectId(req.query.id)};
            MongoUtility.Delete('trending_products', queryObj, (err, product) => {
                if (err) {
