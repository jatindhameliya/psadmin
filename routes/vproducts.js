...
routes.get('/', (req, res, next) => {
	if (req.session.admin_id != '' && req.session.admin_id != undefined) {
		res.render('vproducts/list', { 'ociurl': process.env.S3_IMAGE_URL, 'page': 'vproducts' });
	} else {
		var goto = appURI;
...
routes.get('/add', (req, res, next) => {
	if (req.session.admin_id != '' && req.session.admin_id != undefined) {
		res.render('vproducts/product', { 'ociurl': process.env.S3_IMAGE_URL, 'page': 'vproducts' });
	} else {
		var goto = appURI;
		res.writeHead(302, { 'Location': goto });
		res.end();
...
routes.get('/edit', (req, res, next) => {
	if (req.session.admin_id != '' && req.session.admin_id != undefined) {
		if (req.query.cid != '' && req.query.cid != undefined && req.query.cid != 0 && req.query.cid != null) {
			...
			var goto = process.env.APP_URI + '/vproducts';
			res.redirect(goto);
			res.end();
		}
	} else {
